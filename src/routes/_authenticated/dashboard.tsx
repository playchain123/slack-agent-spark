...
export const Route = createFileRoute("/_authenticated/dashboard")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Trelo: AI answers, commitments, and daily digests from your team's Slack.",
      },
    ],
  }),
  loader: ({ context }) => {
    ...
    >
      ...
      >
        <TopBar
          ...
          setCollapsed={setCollapsed}
          onLogout={logout}
          userEmail={data.profile?.email ?? undefined}
...
function SlackSyncBanner({
  ...
  return (
    <div
      className="mx-6 mt-4 rounded-xl border p-4 flex items-center gap-4"
      style={{
    ...
    >
      <div
        ...
        style={{ background: error ? "#fee2e2" : "#dbeafe" }}
      >
        {syncing ? (
          <Loader2 size={19} className="animate-spin" color={error ? "#991b1b" : "#1d4ed8"} />
        ) : (
          <Slack size={19} color={error ? "#991b1b" : "#1d4ed8"} />
        )}
      </div>
      <div className="flex-1 min-w-0">
        <div className="text-[13px] font-bold" style={{ color: error ? "#991b1b" : "#1e3a8a" }}>
          Slack is connected
        </div>
        <div className="text-[11.5px] mt-0.5" style={{ color: error ? "#991b1b" : "#1d4ed8" }}>
          {text}
        </div>
        {result?.errors && result.errors.length > 0 && (
          <div className="text-[10.5px] mt-1" style={{ color: "#1d4ed8" }}>
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="text-[11.5px] mt-0.5" style={{ color: "#92400e" }}>
          {err ??
            "Install the Trelo Slack app to unlock real answers, commitments, and daily digests."}
        </div>
      </div>
      <button
        onClick={onConnect}
    ...
    >
      <div className="flex items-center gap-2 flex-1 max-w-xl relative">
        ...
        <div className="relative flex-1">
          ...
          {q.trim().length >= 2 && (
            <div
              className="absolute top-full left-0 right-0 mt-1 rounded-lg border bg-white shadow-lg max-h-96 overflow-auto z-50"
              style={{ borderColor: c.outline }}
            >
              {searchQuery.isLoading && (
                <div className="p-3 text-[11px] text-gray-500">Searching…</div>
              )}
              {searchQuery.data && searchQuery.data.length === 0 && (
                <div className="p-3 text-[11px] text-gray-500">No matches.</div>
              )}
      ...
      <div className="flex items-center gap-2 ml-4 relative">
        <button className="p-1.5 rounded-full hover:bg-[#f1edee]">
          <Bell size={15} />
        </button>
        <button className="p-1.5 rounded-full hover:bg-[#f1edee]">
          <HelpCircle size={15} />
        ...
        {openMenu && (
          <div
            className="absolute right-0 top-full mt-1 w-56 rounded-lg border bg-white shadow-lg py-1 z-50"
            style={{ borderColor: c.outline }}
          >
...
function DashboardView({
  ...
  const priority = (commitmentsQuery.data ?? [])
      ...
      <div className="p-6 max-w-[1440px] mx-auto">
        ...
        >
          ...
          <p className="text-[12px] mt-1 max-w-md" style={{ color: c.onSurfaceVariant }}>
            Connect Slack above to fill this dashboard.
          </p>
        </div>
      </div>
    );
    ...
    <div className="p-6 max-w-[1440px] mx-auto">
      <section className="mb-6">
        ...
        <p className="mt-1 text-[13px] max-w-3xl" style={{ color: c.onSurfaceVariant }}>
          Trelo has analyzed <b>{m?.messages24h ?? 0} new Slack threads</b> since you last checked.
          Review the daily digest below and act on your priority commitments.
        </p>
      </section>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        <BigTile
          ...
          value={threadsIndexed.toLocaleString()}
          icon={<Hash size={14} />}
        />
        <BigTile
      ...
      <div className="grid lg:grid-cols-[1.5fr_1fr] gap-4">
          ...
          {digests.length === 0 ? (
            ...
            <div className="space-y-3">
              {digests.slice(0, 4).map((d: any) => (
                <button
                  key={d.id}
                  onClick={() => setOpenDigest(d)}
                  className="w-full text-left rounded-lg border p-3 hover:shadow-sm transition-shadow"
                  style={{ borderColor: c.surfaceMid, background: "#fafafa" }}
                >
                  <div className="flex items-center justify-between gap-3 mb-1.5">
                    ...
                    >
                      #{d.channel_name ?? "channel"}
                    </span>
                    <span className="text-[10.5px] text-gray-500">
                      {formatDistanceToNow(new Date(d.occurred_at), { addSuffix: true })}
                    </span>
                  </div>
                  <div className="text-[12.5px] whitespace-pre-wrap leading-relaxed line-clamp-4">
                    {d.summary}
                  </div>
                  <div className="text-[10.5px] text-gray-500 mt-2 font-semibold">
        ...
        <aside className="space-y-4">
          <div className="rounded-xl border bg-white p-4" style={{ borderColor: c.outline }}>
            ...
            <p className="text-[11.5px] mb-3" style={{ color: c.onSurfaceVariant }}>
              Ask anything about your team's conversations.
            </p>
            <button
              onClick={() => setView("ask")}
              className="w-full h-9 rounded-lg text-[12px] font-semibold text-white"
              style={{ background: "#000" }}
            >
              Ask Trelo →
            </button>
          ...
          <div className="rounded-xl border bg-white p-4" style={{ borderColor: c.outline }}>
            <div className="flex items-center justify-between mb-2">
              <h3 className="text-[13px] font-bold flex items-center gap-2">
                <CheckSquare size={14} /> Priority Commitments
              </h3>
              <span
            ...
            {priority.length === 0 ? (
              ...
              <ul className="space-y-2">
                {priority.map((p: any) => (
                  <li key={p.id}>
                    <button
                      onClick={() => setOpenCommit(p)}
                      className="w-full text-left flex items-start gap-2 text-[12px] hover:bg-gray-50 rounded p-1 -m-1"
                    >
                      <div
                        className="mt-1 w-3.5 h-3.5 rounded border shrink-0"
                        style={{ borderColor: c.outline }}
                      />
            ...
            <button
              onClick={() => setView("commitments")}
              className="mt-3 w-full h-8 rounded-md text-[11px] font-semibold border"
              style={{ borderColor: c.outline }}
            >
  ...
  return (
    <button
      ...
      style={{
        ...
        color: highlight ? "#fff" : c.onSurface,
      }}
    >
      <div
        className="flex items-center gap-1 text-[9.5px] uppercase tracking-wider font-semibold"
        style={{ opacity: highlight ? 0.8 : 0.6 }}
      >
        {icon} {label}
      </div>
...
function EmptyLine({ text }: { text: string }) {
  return (
    <div className="text-[11.5px]" style={{ color: c.onSurfaceVariant }}>
      {text}
    </div>
  );
}
...
function DetailModal({
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-2xl max-h-[85vh] rounded-xl bg-white flex flex-col shadow-2xl"
      ...
      >
        ...
        >
          ...
          <button
            onClick={onClose}
            className="text-gray-400 hover:text-black text-[20px] leading-none px-1"
          >
            ×
          </button>
        </div>
...
function DigestDetailModal({ digest, onClose }: { digest: any | null; onClose: () => void }) {
  return (
    <DetailModal
      ...
      subtitle={
        digest ? formatDistanceToNow(new Date(digest.occurred_at), { addSuffix: true }) : ""
      }
      footer={
        <a
          href={digest?.permalink ?? "https://slack.com"}
          target="_blank"
          rel="noreferrer"
          className="h-9 px-3 rounded-md text-[12px] font-semibold text-white flex items-center gap-1.5"
          style={{ background: "#000" }}
        >
          ...
          <div className="space-y-2">
            {sources.map((s: any, i: number) => (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="block text-[12px] hover:underline p-2 rounded border"
                style={{ borderColor: c.surfaceMid }}
              >
                <span className="text-gray-400">[{s.index ?? i + 1}]</span>{" "}
                <span className="font-semibold">#{s.channel ?? "channel"}</span> —{" "}
...
function CommitmentDetailModal({
  item,
  onClose,
  onAccept,
  onToggle,
        ...
        <>
          {item.source_permalink && (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="h-9 px-3 rounded-md text-[12px] font-semibold border flex items-center gap-1.5"
              style={{ borderColor: c.outline }}
            >
          ...
          {item.status === "pending" && onToggle && (
            <button
              ...
              className="h-9 px-3 rounded-md text-[12px] font-semibold text-white"
              style={{ background: "#000" }}
            >
              Mark done
            </button>
          ...
          {onDelete && (
            <button
              ...
              className="h-9 px-3 rounded-md text-[12px] font-semibold border text-red-600"
              style={{ borderColor: c.outline }}
            >
              Delete
            </button>
    ...
    <div className="p-6 max-w-[1440px] mx-auto">
      <div className="grid lg:grid-cols-[1fr_320px] gap-6">
        <section className="min-w-0">
          ...
          >
            <div
              className="flex gap-2 rounded-full border bg-white p-1.5 shadow-sm"
              style={{ borderColor: c.outline }}
            >
              ...
              <input
                ...
                placeholder={
                  isConnected
                    ? "Ask anything your team has discussed in Slack…"
                    : "Connect Slack first…"
                }
              ...
              <button
                type="submit"
                disabled={!isConnected || mutation.isPending || !input.trim()}
            ...
            <div className="flex flex-wrap gap-2 mt-3 justify-center">
              ...
              {EXAMPLES.map((ex) => (
                <button
                  ...
                  type="button"
                  onClick={() => setInput(ex)}
                  className="text-[11.5px] px-2.5 py-1 rounded-full border hover:bg-gray-50"
                  style={{ borderColor: c.outline }}
                >
                  "{ex}"
                </button>
          ...
          {mutation.isError && (
            <div
              className="mt-6 rounded-lg border p-3 text-[12px] max-w-2xl mx-auto"
          ...
          {mutation.data && (
            <div className="mt-6 max-w-3xl mx-auto">
              <AnswerCard
                question={lastQuestion || "Your question"}
          ...
          <div className="mt-10">
            ...
            {!historyQuery.isLoading && answers.length === 0 && (
              <div
                className="rounded-xl border bg-white p-6 text-[12px] text-center"
                style={{ color: c.onSurfaceVariant, borderColor: c.outline }}
              >
        ...
        <aside className="space-y-4">
                ...
                {trending.map((t: any) => (
                  <li key={t.id}>
                    ...
                    >
                      ...
                      <div className="text-[12px] font-semibold line-clamp-2 mt-0.5">
                        {t.question}
                      </div>
          ...
          <div
            ...
            style={{ borderColor: c.outline, background: "#000", color: "#fff" }}
          >
            <div className="flex items-center gap-2 mb-1.5">
              <Sparkles size={14} />
              <h3 className="text-[13px] font-bold">Power Tip</h3>
            </div>
            <p className="text-[11.5px] opacity-80">
              Mention{" "}
              <code className="px-1 rounded" style={{ background: "#333" }}>
                @trelo
              </code>{" "}
              in any Slack channel to get answers right in the thread.
            </p>
          </div>
        </aside>
      </div>
...
function AnswerCard({
  ...
  return (
    <div className="rounded-xl border p-4 bg-white" style={{ borderColor: c.outline }}>
      ...
      <div
        className={`text-[12.5px] whitespace-pre-wrap leading-relaxed ${compact ? "line-clamp-4" : ""}`}
...
function CommitmentsView({ isConnected }: { isConnected: boolean }) {
  ...
  const overdue = all
    .filter(
      (c: any) => c.status === "pending" && c.due_date && new Date(c.due_date).getTime() < now,
    )
    .slice(0, 3);
  const pending = all.filter(
    (c: any) => c.status === "pending" && !overdue.some((o: any) => o.id === c.id),
  );
    ...
    <div className="p-6 max-w-[1440px] mx-auto">
      <div className="mb-4 flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-[12.5px]" style={{ color: c.onSurfaceVariant }}>
            Action items extracted from your Slack conversations by Trelo AI.
          </p>
        </div>
        <button
          onClick={() => suggest.mutate()}
          disabled={!isConnected || suggest.isPending}
      ...
      {overdue.length > 0 && (
        <section className="mb-6">
          <h3 className="text-[11px] uppercase tracking-wider font-bold text-gray-500 mb-2 flex items-center gap-2">
            <span
              className="w-4 h-4 rounded-full grid place-items-center text-white text-[9px] font-black"
              style={{ background: "#000" }}
            >
              !
            </span>
          ...
          <div className="grid md:grid-cols-3 gap-3">
            {overdue.map((row: any) => (
              <button
                ...
                className="text-left rounded-xl border p-4 bg-white border-l-4 hover:shadow-sm transition-shadow"
                style={{ borderColor: c.outline, borderLeftColor: "#000" }}
              >
                <div className="flex items-center justify-between mb-2">
                  <span
                  ...
                  <span className="text-[10.5px] text-gray-500">
                    {formatDistanceToNow(new Date(row.due_date), { addSuffix: true })}
                  </span>
                </div>
                <div className="text-[13px] font-bold mb-1">{row.title}</div>
                {row.channel_name && (
                  <div className="text-[10.5px] text-gray-500 mb-2">#{row.channel_name}</div>
                )}
                <div
      ...
      >
        ...
        <button
          ...
          className="h-10 px-3 rounded-lg text-[12px] font-semibold text-white disabled:opacity-50"
          style={{ background: "#000" }}
        >
      ...
      {suggested.length > 0 && (
        <section className="mb-6">
          <h3 className="text-[11px] uppercase tracking-wider font-bold text-gray-500 mb-2 flex items-center gap-2">
            ...
            <span
              className="text-[10px] px-1.5 rounded-full font-bold border"
              style={{ borderColor: c.outline }}
            >
              {suggested.length}
            </span>
          ...
          <ul className="space-y-2">
            {suggested.map((row: any) => (
              <li
                key={row.id}
                className="rounded-lg border p-3 bg-white flex items-start gap-3"
                style={{ borderColor: c.outline }}
              >
                ...
                <button
                  ...
                  className="px-2 py-1 rounded-md text-[11px] font-semibold text-white flex items-center gap-1"
                  style={{ background: "#000" }}
                >
                  <Check size={12} /> Add
                </button>
      ...
      <section className="mb-6">
        ...
        {pending.length === 0 && !listQuery.isLoading && (
          <div
            className="rounded-xl border p-8 bg-white text-center"
            style={{ borderColor: c.outline }}
          >
        ...
        <ul className="space-y-2">
          {pending.map((row: any) => (
            <li
              key={row.id}
              className="rounded-lg border p-3 bg-white flex items-center gap-3"
              style={{ borderColor: c.outline }}
            >
              <input
                type="checkbox"
      ...
      <section>
        <button
          ...
          className="text-[11px] uppercase tracking-wider font-bold text-gray-500 mb-2 flex items-center gap-2"
        >
        ...
        {completedOpen && (
          <ul className="space-y-2">
            {done.map((row: any) => (
              <li
                key={row.id}
                className="rounded-lg border p-3 bg-white flex items-center gap-3 opacity-60"
                style={{ borderColor: c.outline }}
              >
                <input
                  type="checkbox"
                ...
                <button
                  onClick={() => setOpenItem(row)}
                  className="flex-1 min-w-0 text-[12.5px] line-through truncate text-left"
                >
                  {row.title}
                </button>
                <span
...
function DigestView({ isConnected }: { isConnected: boolean }) {
  ...
  const channelSet = Array.from(
    new Set(all.map((d: any) => d.channel_name).filter(Boolean)),
  ) as string[];
      ...
      <div className="grid lg:grid-cols-[1fr_320px] gap-6">
        <section className="min-w-0">
          <div className="mb-4 flex items-start justify-between gap-4">
            <div>
              ...
              <p className="text-[12.5px]" style={{ color: c.onSurfaceVariant }}>
                Intelligent summaries of your team's Slack activity.
              </p>
            </div>
            <button
              onClick={() => gen.mutate()}
              disabled={!isConnected || gen.isPending}
              className="h-9 px-3 rounded-lg text-[12px] font-semibold text-white flex items-center gap-1.5 disabled:opacity-50"
              style={{ background: "#000" }}
            >
              {gen.isPending ? (
          ...
          <div className="flex items-center gap-2 mb-4 flex-wrap">
            <div
              className="flex gap-1 rounded-lg border p-0.5 bg-white text-[11px]"
              style={{ borderColor: c.outline }}
            >
              {(
                [
                ...
                <button
                  ...
                  style={
                    range === id ? { background: "#000", color: "#fff" } : { color: c.onSurface }
                  }
                >
                  {label}
                </button>
            ...
            <select
              ...
              className="h-8 rounded-lg border bg-white px-2 text-[11.5px] outline-none"
              style={{ borderColor: c.outline }}
            >
          ...
          <div className="space-y-3">
            {filtered.map((d: any) => (
              <button
                key={d.id}
                onClick={() => setOpenDigest(d)}
                className="w-full text-left rounded-xl border bg-white p-4 hover:shadow-sm transition-shadow"
                style={{ borderColor: c.outline }}
              >
                ...
                <div className="text-[12.5px] whitespace-pre-wrap leading-relaxed line-clamp-5">
                  {d.summary}
                </div>
              </button>
            ))}
        ...
        <aside className="space-y-4">
          <div className="rounded-xl border bg-white p-4" style={{ borderColor: c.outline }}>
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <div className="rounded-lg p-3" style={{ background: c.surfaceLow }}>
                ...
                <div className="text-[22px] font-black leading-none mt-1">
                  {Math.max(participantCount, channelSet.length)}
                </div>
          ...
          <div className="rounded-xl border bg-white p-4" style={{ borderColor: c.outline }}>
            ...
            <div className="text-[10.5px] uppercase tracking-wider text-gray-500 font-semibold mb-2">
              Top Active Channels
            </div>
            <ul className="space-y-2">
              {topChannels.map((ch) => (
                <li key={ch.name} className="flex items-center justify-between text-[12px]">
                  ...
                  <span className="text-gray-500 text-[10.5px]">
                    {ch.count} digest{ch.count === 1 ? "" : "s"}
                  </span>
                </li>
              ))}
              {topChannels.length === 0 && (
                <li className="text-[11.5px] text-gray-500">No data yet.</li>
              )}
            </ul>
          </div>
          <div
